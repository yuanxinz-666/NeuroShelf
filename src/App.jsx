...
import { PaperExperimentDialog } from './ExperimentReferences';
import { indexExperimentReferences, matchesIndexedExperiment, experimentPath } from './experiment-references.mjs';
import referenceRoles from '../data/experiment-reference-roles.json';
import { useWorkspaceShortcuts, ShortcutHelp, pdfCommand } from './WorkspaceShortcuts';
import { Keyboard, Undo2, Redo2, Save as SaveIcon } from 'lucide-react';
import './focus.css';
...
function Sidebar({ project, projectList, switchProject, newProject, view, setView, reading, exitReader, papers, weeklyCount, filterModule, setFilterModule, onImport, onSettings, onBackup }) {
  const panel = usePanelWidth({ name: reading ? 'navigation-reading' : 'navigation', initial: reading ? 69 : 222, min: reading ? 69 : 190, max: 360, reserve: 720, side: 'left', measureParent: true });
  const compact = reading && panel.width < 180;
  ...
  return <div ref={panel.ref} className="navigation-panel" style={{ width: panel.width + 9 }}><aside style={{ width: panel.width }} className={`sidebar ${compact ? 'compact' : ''}`}><Brand compact={compact} />{!compact && <ProjectPicker active={project} projects={projectList} onSwitch={switchProject} onNew={newProject} />}
    {!compact && <div className="nav-label">{t("工作空间")}</div>}<nav>{items.map(([id, Icon, label, count]) => <button title={label} key={id} className={`nav-item ${view === id && !filterModule ? 'active' : ''}`} onClick={() => navigate(id)}><Icon size={18} />{!compact && <><span>{label}</span>{count !== null && <small>{count}</small>}</>}</button>)}</nav>
    {!compact && <><div className="nav-label modules-label">{t("研究模块 ")}<span>{modules.length}</span></div><div className="module-nav">{modules.map((name, i) => <button key={name} className={filterModule === name ? 'selected' : ''} onClick={() => { navigate('library'); setFilterModule(name); }}><i style={{ background: moduleColors[i % moduleColors.length] }} /><span>{t(name)}</span><small>{papers.filter(p => p.module === name).length}</small></button>)}</div></>}
    <div className="sidebar-bottom">{!compact && <div className="local-card"><HardDrive size={16} /><div><strong>{t("为你的研究，留在本地")}</strong><span>{t("PDF · 笔记 · 阅读进度")}</span></div><span className="status-dot" /></div>}<button title={t("导入 PDF")} className="nav-item" onClick={onImport}><Upload size={17} />{!compact && <span>{t("导入 PDF")}</span>}</button><button title={t("备份与迁移")} className="nav-item" onClick={onBackup}><ArrowDownToLine size={17} />{!compact && <span>{t("备份与迁移")}</span>}</button><button title={t("设置")} className="nav-item" onClick={onSettings}><Settings2 size={17} />{!compact && <span>{t("设置与 AI 连接")}</span>}</button>{!compact && <div className="sidebar-version">{t("个人阅读工作台 ")}<span>v{version}</span></div>}</div></aside><ResizeHandle panel={panel} label={t("调整项目导航宽度")} className="navigation-resizer" /></div>;
}
function CircuitArt() {
...
function PaperCard({ paper, open, update, onImport, experimentLinks, onReferences }) {
  return <article className="paper-card"><div className="card-meta"><span className="paper-rank">{rankLabel(paper)}</span><span>{paper.year || t("新导入")}</span><span className="card-journal">{paper.journal || t("本地文献")}</span><button className={`icon-button star-button ${paper.starred ? 'starred' : ''}`} aria-label={`${paper.starred ? t("取消收藏") : t("收藏")} ${paper.title}`} onClick={() => update(paper.id, { starred: !paper.starred }).catch(() => {})}><Star size={16} fill={paper.starred ? 'currentColor' : 'none'} /></button></div>
    ...
    <div className="card-tags"><span>{t(paper.module)}</span>{paper.publication?.includes('预印本') && <span className="preprint">{t("预印本")}</span>}{paper.rank <= 30 && <span className="core-tag">{t("核心必读")}</span>}</div><PaperReview paper={paper} update={update} /><div className="paper-experiment-row"><button onClick={() => onReferences(paper)} aria-label={tx`整理实验用途：${paper.title}`}><FlaskConical size={13} /><span>{experimentLinks.length ? experimentLinks.map(({node}) => node.title).join(' · ') : t('关联到实验')} ({experimentLinks.length})</span><Plus size={12} /></button>{experimentLinks.find(({ref}) => ref.note)?.ref.note && <p>{experimentLinks.find(({ref}) => ref.note).ref.note}</p>}</div><div className="card-bottom"><span className={`reading-state ${paper.status}`}><i />{statusNames[paper.status] || t("未开始")}</span>{paper.pdf ? <button className="pdf-linked" onClick={() => open(paper)}><FileText size={13} />{t("阅读 PDF")}<ArrowRight size={14} /></button> : <button className="add-pdf" onClick={() => onImport(paper.id)}><Plus size={13} />{t("添加 PDF")}</button>}</div></article>;
}
function LibraryView({ project, library, view, module, query, setQuery, papers, open, update, onImport, experimentFilter, setExperimentFilter, referenceRole, setReferenceRole, onReferences }) {
  const panel = usePanelWidth({ name: "library-sidebar", initial: 247, min: 180, max: 420, reserve: 330 });
  const [tab, setTab] = useState('all'), [sort, setSort] = useState('rank'), [publication, setPublication] = useState('');
  const [visible, setVisible] = useState(20);
  const experimentNodes = library.experiments?.nodes || [];
  const referenceIndex = useMemo(() => indexExperimentReferences(experimentNodes), [library.experiments]);
  useEffect(() => setVisible(20), [query, tab, sort, module, view, publication, experimentFilter, referenceRole]);
  const filtered = useMemo(() => {
    ...
    return papers.filter(p => (!module || p.module === module) && (view !== 'starred' || p.starred) && (view !== 'reading' || p.status === 'reading') && (view !== 'read' || p.status === 'read') && (view !== 'notes' || p.personalReview || p.note || p.highlights?.length)
      && matchesIndexedExperiment(referenceIndex, p.id, experimentFilter, referenceRole) && (tab !== 'core' || p.rank <= 30) && (tab !== 'pdf' || p.pdf) && (!publication || p.publication === publication)
      && (!term || [p.title, p.authors, p.journal, p.summary, p.relevance, p.caveat, p.personalReview, p.note, p.tags, ...(referenceIndex.linksByPaper.get(p.id) || []).flatMap(({node,ref}) => [node.title,ref.note]), ...(p.highlights || []).flatMap(h => [h.text, h.comment || ''])].join(' ').toLowerCase().includes(term)))
      .sort((a, b) => sort === 'year' ? (b.year || 0) - (a.year || 0) : sort === 'recent' ? (Date.parse(b.updatedAt) || 0) - (Date.parse(a.updatedAt) || 0) : a.rank - b.rank);
  }, [papers, module, view, tab, sort, query, publication, library.experiments, experimentFilter, referenceRole]);
  ...
  return <div className="library-scroll"><div className="library-page"><div className="page-eyebrow">YOUR RESEARCH, CONNECTED</div><div className="library-title"><div><h1>{t(title)}</h1><p>{t("从一篇论文，到一个更清晰的研究问题。")}</p></div><button className="button primary" onClick={() => onImport()}><Plus size={16} />{t("导入论文 PDF")}</button></div>
    ...
    <div ref={panel.ref} className="library-content" style={{ gridTemplateColumns: `minmax(0, 1fr) 9px ${panel.width}px` }}><div className="paper-list-area"><div className="library-tabs">{[['all', t("全部文献")], ['core', t("核心必读")], ['pdf', t("已有 PDF")]].map(([id, label]) => <button className={tab === id ? 'active' : ''} key={id} onClick={() => setTab(id)}>{label}{id === 'all' && <span>{papers.length}</span>}</button>)}<select aria-label={t("文献排序")} value={sort} onChange={e => setSort(e.target.value)}><option value="rank">{t("项目相关性")}</option><option value="year">{t("发表年份 ↓")}</option><option value="recent">{t("最近记录")}</option></select></div>
      <div className="experiment-library-filter"><label><FlaskConical size={15} />{t('按实验整理')}<select title={t('含子步骤')} aria-label={t('按实验筛选文献')} value={experimentFilter} onChange={e => { setExperimentFilter(e.target.value); if (e.target.value === 'unassigned') setReferenceRole(''); }}><option value="">{t('全部实验')}</option><option value="unassigned">{t('尚未关联实验')}</option>{experimentNodes.filter(n => !n.archived).map(n => <option key={n.id} value={n.id}>{experimentPath(experimentNodes, n.id)} ({referenceIndex.papersByExperiment.get(n.id)?.size || 0})</option>)}</select></label><select aria-label={t('按文献用途筛选')} value={referenceRole} disabled={experimentFilter === 'unassigned'} onChange={e => setReferenceRole(e.target.value)}><option value="">{t('全部文献用途')}</option>{Object.entries(referenceRoles).map(([key,label]) => <option value={key} key={key}>{t(label)}</option>)}</select></div>
      ...
      <div className="paper-grid">{filtered.slice(0, visible).map(p => <PaperCard key={p.id} paper={p} open={open} update={update} onImport={onImport} experimentLinks={(referenceIndex.linksByPaper.get(p.id) || [])} onReferences={onReferences} />)}</div>
      {!filtered.length && <div className="empty-state"><Search size={30} /><h3>{t("这里还没有文献")}</h3><p>{t("试试其他关键词，或导入一篇新的 PDF。")}</p><button className="button" onClick={() => { setQuery(''); setTab('all'); setPublication(''); setExperimentFilter(''); setReferenceRole(''); }}>{t("清除搜索和筛选")}</button></div>}
...
function Guide({ paper, update, onImport, onRead, onAsk, onContext, notify }) {
  ...
  saveRef.current = async () => { if (editing) await saveMetadata(); };
  useEffect(() => registerDraftFlusher(`metadata:${paper.id}`, () => saveRef.current()), [paper.id]);
  useEffect(() => { if (!editing) { setTitle(paper.title); setAuthors(paper.authors || ''); setJournal(paper.journal || ''); } }, [paper.title, paper.authors, paper.journal, editing]);
...
function Notes({ paper, update, mutateHighlight, notify, onJump, onAnnotation }) {
  ...
  useEffect(() => registerDraftFlusher(paper.id, async () => {
    clearTimeout(timer.current);
    if (draft.current) {
      const snapshot = draft.current; setState('保存中…');
      try { await updateRef.current(paper.id, snapshot); if (draft.current === snapshot) draft.current = null; setState(draft.current ? '待保存' : '已保存'); }
      catch (error) { setState('保存失败，请重试'); throw error; }
    }
  }), [paper.id]);
...
function SettingsModal({ settings, onSettings, onClose, notify }) {
  ...
  return <Modal title={t("连接你的阅读助手")} subtitle={t("使用已有 ChatGPT／Codex 会员，即可在论文旁边提问。")} onClose={onClose}>
    ...
    </div><div className="modal-footer"><button className="text-button" onClick={() => api.openExternal(provider === 'api' ? 'https://platform.openai.com/api-keys' : 'https://learn.chatgpt.com/docs/auth')}>{provider === 'api' ? t("API 密钥页面") : t("官方登录说明")}<ExternalLink size={12} /></button><button className="button primary" data-shortcut-save disabled={!api.desktop || busy} onClick={save}>{busy ? <LoaderCircle className="spin" size={15} /> : <Check size={15} />}{t("保存设置")}</button></div>
  </Modal>;
}
function BackupModal({ library, projectState, chooseRoot, onClose, onLibrary, notify }) {
  const [busy, setBusy] = useState(false), [error, setError] = useState('');
...
export default function App() {
  ...
  const [referencePaper, setReferencePaper] = useState(null), [experimentSelected, setExperimentSelected] = useState(null), [experimentFilter, setExperimentFilter] = useState(''), [referenceRole, setReferenceRole] = useState('');
  const [focusMode, setFocusMode] = useState(false), [focusPanel, setFocusPanel] = useState(false), [focusSide, setFocusSide] = useState('right');
  const focusBusy = useRef(false), historyBusy = useRef(false);
  ...
  const update = useCallback((id, patch, options) => { const promise = api.update(id, patch, options).then(paper => { onPaper(paper); return paper; }).catch(e => { notify(t("保存失败：") + e.message); throw e; }); pendingSaves.current.add(promise); promise.then(() => pendingSaves.current.delete(promise), () => pendingSaves.current.delete(promise)); return promise; }, [onPaper, notify]);
  ...
  async function chooseProjectRoot() {
    await flushDrafts();
    while (pendingSaves.current.size) await Promise.all([...pendingSaves.current]);
    if (weeklyRequest.current) await weeklyRequest.current;
    const selected = await api.chooseProjectRoot();
  ...
  useEffect(() => { const handler = e => { if (e.key === 'Escape' && !document.querySelector('dialog[open]')) setToast(''); }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler); }, []);
  const paper = library?.papers.find(p => p.id === openedId);
  async function saveNow(silent = false) {
    await flushDrafts();
    while (pendingSaves.current.size) await Promise.all([...pendingSaves.current]);
    await api.flush?.();
    if (!silent) notify(t('当前文献与实验记录已保存'));
  }
  async function historyAction(direction) {
    if (historyBusy.current || !api.historyAction) return;
    historyBusy.current = true;
    let work;
    try {
      await saveNow(true); work = api.historyAction(direction); pendingSaves.current.add(work);
      const result = await work; if (result.changed) setLibrary(result.library);
      notify(result.changed ? t(direction === 'undo' ? '已撤销' : '已重做') : t(direction === 'undo' ? '暂无可撤销的编辑' : '暂无可重做的编辑'));
    } catch (error) { notify(error.message); }
    finally { if (work) pendingSaves.current.delete(work); historyBusy.current = false; }
  }
  async function toggleFocus() {
    if (focusBusy.current) return;
    if (!paper?.pdf) { notify(t('请先打开一篇 PDF，再进入专注阅读。')); return; }
    focusBusy.current = true;
    try {
      await saveNow(true); const next = !focusMode;
      await api.setFocusMode?.(next); setReaderTab('pdf'); setFocusMode(next); setFocusPanel(false);
      document.activeElement?.blur(); pdfCommand('clear');
    } catch (error) { notify(error.message); }
    finally { focusBusy.current = false; }
  }
  async function closeFocusPanel() { try { await saveNow(true); setFocusPanel(false); pdfCommand('clear'); document.activeElement?.blur(); } catch (error) { notify(error.message); } }
  async function leaveFocusThen(action) { if (focusMode) { await saveNow(true); await api.setFocusMode?.(false); setFocusMode(false); setFocusPanel(false); } requestAnimationFrame(action); }
  useEffect(() => api.onFocusMode?.(value => { if (!value) { setFocusMode(false); setFocusPanel(false); } }), []);
  useEffect(() => { if (focusMode && (!paper?.pdf || readerTab !== 'pdf')) { api.setFocusMode?.(false).catch(e => notify(e.message)); setFocusMode(false); setFocusPanel(false); } }, [paper?.id, readerTab]);
  useWorkspaceShortcuts({
    pdf: Boolean(paper?.pdf && readerTab === 'pdf'), focus: focusMode,
    save: () => saveNow().catch(e => notify(e.message)), history: historyAction, toggleFocus,
    escapeFocus: () => { if (focusPanel) closeFocusPanel(); else if (document.querySelector('.selection-toolbar')) pdfCommand('clear'); else toggleFocus(); },
    find: () => paper?.pdf && readerTab === 'pdf' ? pdfCommand('find') : searchRef.current?.focus(),
    search: () => leaveFocusThen(() => searchRef.current?.focus()).catch(e => notify(e.message)),
    importPdf: () => leaveFocusThen(() => onImport()).catch(e => notify(e.message)), help: () => setModal('shortcuts'),
  });
  function openExperiment(id) { setOpenedId(null); setView('experiments'); setQuery(''); setExperimentSelected(id); }
  function open(p) { if (!p) return; setOpenedId(p.id); setReaderTab(p.pdf ? 'pdf' : 'guide'); setContext(null); setAskDraft(null); setJump(null); setActiveAnnotation(null); if (p.status === 'unread' || p.status === 'queued') update(p.id, { status: 'reading' }, { history: false }).catch(() => {}); }
  function onImport(id = null) { setImportTarget(id); setModal('import'); }
  function ask(text, options = {}) {
    setSidebarTab('ai');
    if (focusMode) { setFocusSide((options.context?.anchor?.x || 0) > window.innerWidth / 2 ? 'left' : 'right'); setFocusPanel(true); }
  ...
  function annotate(highlight, focus = false) {
    ...
    if (focusMode) { setFocusSide((context?.anchor?.x || 0) > window.innerWidth / 2 ? 'left' : 'right'); setFocusPanel(true); }
  ...
  return <div className={`app-shell ${paper ? 'reading-mode' : ''} ${focusMode ? 'focus-mode' : ''}`}><Sidebar key={paper ? "reading" : "workspace"} project={project} projectList={projectState.projects} switchProject={id => switchProject(id).catch(() => {})} newProject={() => setModal('new-project')} papers={library.papers} weeklyCount={(library.weekly?.candidates || []).filter(c => c.status === 'pending').length} view={view} setView={next => { const go = () => { setView(next); if (next === 'weekly' || next === 'experiments' || view === 'experiments') setQuery(''); }; if (view === 'experiments') flushDrafts().then(go).catch(e => notify(e.message)); else go(); }} reading={!!paper} exitReader={() => { setOpenedId(null); setContext(null); }} filterModule={filterModule} setFilterModule={setFilterModule} onImport={() => onImport()} onSettings={() => setModal('settings')} onBackup={() => setModal('backup')} /><main className="main-workspace"><header className="topbar"><div className="breadcrumbs">{paper ? <button onClick={() => { setOpenedId(null); setContext(null); }}><ArrowLeft size={15} />{view === 'weekly' ? t("返回每周新论文") : view === 'experiments' ? t('返回实验进程') : t("返回文献库")}</button> : <><span>{project.name}</span><ChevronRight size={13} /><b>{view === 'project' ? t("项目总览") : view === 'experiments' ? t("实验进程") : view === 'people' ? t("PI 与实验室") : view === 'weekly' ? t("每周新论文") : view === 'map' ? t("研究地图") : t("文献库")}</b></>}</div><div className="global-search"><Search size={16} /><input ref={searchRef} aria-label={t("搜索文献库")} placeholder={view === 'experiments' && !paper ? t("搜索实验步骤、结果、图片说明…") : view === 'people' && !paper ? t("搜索 PI、机构与研究方向…") : view === 'weekly' && !paper ? t("搜索候选论文、推荐理由…") : t("搜索论文、作者、评价、笔记…")} value={query} onChange={e => { setQuery(e.target.value); if (paper || view === 'map' || view === 'project') { setOpenedId(null); setView('library'); } }} /><kbd>Ctrl K</kbd>{query && <button className="icon-button" aria-label={t("清除搜索")} onClick={() => setQuery('')}><X size={13} /></button>}</div><div className="topbar-right"><div className="workspace-actions"><button className="icon-button" aria-label={t('立即保存当前记录')} title="Ctrl S" onClick={() => saveNow().catch(e => notify(e.message))}><SaveIcon size={16} /></button><button className="icon-button" aria-label={t('撤销上一步编辑')} title="Ctrl Z" onClick={() => historyAction('undo')}><Undo2 size={16} /></button><button className="icon-button" aria-label={t('重做上一步编辑')} title="Ctrl Y" onClick={() => historyAction('redo')}><Redo2 size={16} /></button><button className="icon-button" aria-label={t('键盘快捷键')} title="F1" onClick={() => setModal('shortcuts')}><Keyboard size={17} /></button></div><LanguageSettings compact /><span className="local-indicator"><span className="status-dot" />{api.desktop ? t("本地资料库") : t("浏览器预览")}</span><button className="user-avatar" title={t("设置")} onClick={() => setModal('settings')}>YZ</button></div></header>
      ...
      </section></ReaderWorkbench></> : view === 'project' ? <ProjectHub project={project} library={library} job={job} start={startResearch} stop={stopResearch} navigate={next => { setView(next); setQuery(''); }} edit={() => setModal('edit-project')} schedule={weeklySyncState.schedule} notify={notify} /> : view === 'experiments' ? <Experiments key={project.id} papers={library.papers} open={open} initialSelectedId={experimentSelected} onSelect={setExperimentSelected} project={project} data={library.experiments} change={payload => saveExperiment('changeExperiment', payload)} importImage={payload => saveExperiment('importEvidence', payload)} query={query} notify={notify} /> : view === 'people' ? <PeopleView project={project} startPi={startPiResearch} job={job} stop={stopResearch} refresh={reloadLibrary} people={library.people} update={updatePerson} start={startResearch} busy={['running', 'stopping'].includes(job?.status)} query={query} notify={notify} /> : view === 'weekly' ? <WeeklyInbox project={project} weekly={library.weekly} syncState={weeklySyncState} sync={syncWeekly} decide={decideWeekly} papers={library.papers} open={open} onImport={onImport} query={query} setQuery={setQuery} notify={notify} /> : view === 'map' ? <ResearchMap library={library} open={open} /> : <LibraryView experimentFilter={experimentFilter} setExperimentFilter={setExperimentFilter} referenceRole={referenceRole} setReferenceRole={setReferenceRole} onReferences={setReferencePaper} project={project} library={library} view={view} module={filterModule} query={query} setQuery={setQuery} papers={library.papers} open={open} update={update} onImport={onImport} />}
    </main>{['new-project', 'edit-project'].includes(modal) && <ProjectDialog project={modal === 'edit-project' ? project : null} onClose={() => setModal(null)} onSave={saveProject} />}{projectBusy && <div className="project-switching" role="status"><LoaderCircle className="spin" />{t("正在保存并切换项目…")}</div>}{modal === 'shortcuts' && <ShortcutHelp close={() => setModal(null)} />}{modal === 'import' && <ImportModal papers={library.papers} target={importTarget} onClose={() => setModal(null)} onImported={onPaper} notify={notify} />}{referencePaper && <PaperExperimentDialog paper={library.papers.find(p => p.id === referencePaper.id) || referencePaper} nodes={library.experiments?.nodes || []} change={payload => saveExperiment('changeExperiment', payload)} close={() => setReferencePaper(null)} openExperiment={openExperiment} />}
    {modal === 'settings' && <SettingsModal settings={settings} onSettings={setSettings} onClose={() => setModal(null)} notify={notify} />}{modal === 'backup' && <BackupModal library={library} projectState={projectState} chooseRoot={chooseProjectRoot} onClose={() => setModal(null)} onLibrary={setLibrary} notify={notify} />}
