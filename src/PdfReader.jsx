...
const EXPLAIN_SELECTION = '这段原文是什么意思？请用中文先直白解释，再说明关键术语及其在当前论文中的含义；结合提供的上下文，区分原文证据和推断。';
...
export default function PdfReader({ paper, update, onContext, onAsk, onImport, notify, jump, mutateHighlight, onAnnotation, activeAnnotation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setError(e.name === 'PasswordException' ? '这份 PDF 有密码保护。请先用 PDF 软件解锁后重新导入。' : 'PDF 无法打开：' + e.message); setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setRendering(false);
      } catch (e) { if (!cancelled && e.name !== 'RenderingCancelledException' && e.name !== 'AbortException') { setError('此页渲染失败：' + e.message); setRendering(false); } }
    })();
  ...
  function readerContext(excerpt = selection?.text || '') {
    return { page, totalPages: doc?.numPages || 0, pageText: text, selection: excerpt, source: 'pdf',
      ...
      getImage: () => {
        ...
        if (!original?.width || rendering) throw new Error('请等待当前页显示完成。');
  ...
  useEffect(() => {
    const explainOnSpace = event => {
      ...
      onAsk(EXPLAIN_SELECTION, { submit: true, context: readerContext(selected.text) });
    };
  ...
  function findText() {
    ...
    setHits(result);
    if (!result.length) { setHitIndex(-1); notify('已解析页面中没有找到该词'); return; }
    const next = search && hits.join() === result.join() ? (hitIndex + 1) % result.length : 0;
    setHitIndex(next); go(result[next]);
  }
  if (loading) return <div className="reader-loading"><LoaderCircle className="spin" size={28} /><p>正在打开 PDF…</p></div>;
  if (!doc) return <div className="reader-loading"><FileWarning size={34} /><h3>暂时无法阅读这份 PDF</h3><p>{error}</p><button className="button" onClick={onImport}><Upload size={16} />导入其他 PDF</button></div>;
  ...
  return <div className="pdf-reader">
    <div className="pdf-toolbar">
      <div className="toolbar-group"><button className="icon-button" aria-label="上一页" disabled={page <= 1} onClick={() => go(page - 1)}><ChevronLeft size={17} /></button>
        <input aria-label="PDF 页码" className="page-input" value={pageInput} onChange={e => setPageInput(e.target.value)} onBlur={() => go(pageInput)} onKeyDown={e => { if (e.key === 'Enter') go(pageInput); }} />
        <span className="muted">/ {doc.numPages}</span><button className="icon-button" aria-label="下一页" disabled={page >= doc.numPages} onClick={() => go(page + 1)}><ChevronRight size={17} /></button></div>
      <div className="toolbar-group"><button className="icon-button" aria-label="缩小" disabled={zoom <= .6} onClick={() => setZoom(v => Math.max(.6, v - .15))}><Minus size={16} /></button><span className="zoom-label">{Math.round(zoom * 100)}%</span><button className="icon-button" aria-label="放大" disabled={zoom >= 2.2} onClick={() => setZoom(v => Math.min(2.2, v + .15))}><Plus size={16} /></button><button className="icon-button" title="适应宽度" aria-label="适应宽度" onClick={() => setZoom(1)}><Scan size={16} /></button></div>
      <form className="pdf-search" onSubmit={e => { e.preventDefault(); findText(); }}><Search size={15} /><input aria-label="搜索 PDF 全文" value={search} onChange={e => setSearch(e.target.value)} placeholder="查找原文…" /><button type="submit" title="查找下一页">{hits.length ? `${hitIndex + 1}/${hits.length}` : '↵'}</button></form>
    </div>
    {error && <div className="inline-error">{error}</div>}
    <div className="pdf-scroll" ref={scrollRef}>
      <div className="pdf-page" ref={pageRef} onMouseUp={captureSelection} onTouchEnd={captureSelection}>
        <canvas ref={canvasRef} aria-label={`论文 PDF 第 ${page} 页`} />
        ...
        <div className="annotation-markers">{pageHighlights.filter(h => h.rects?.length).map((h, index) => <button key={h.id} className={activeAnnotation?.id === h.id ? 'active' : ''} style={{ top: `${h.rects[0].y * 100}%` }} aria-label={`查看第 ${index + 1} 条原文旁注`} title={h.comment || '为这段高亮写旁注'} onClick={() => onAnnotation(h, true)}><MessageSquareText size={15} /></button>)}</div>
      </div>
      {rendering && <div className="render-status"><LoaderCircle size={14} className="spin" />正在显示第 {page} 页</div>}
      {!rendering && !text.trim() && <div className="scan-notice"><ImageIcon size={16} />此页没有可选文字。可在右侧勾选「附上当前页图片」，让 AI 解释扫描页或图表。</div>}
    </div>
    {selection && <div className="selection-toolbar" onMouseDown={e => e.preventDefault()}><span>已选 {selection.text.length} 字</span><button title="直接让 AI 解释选区（空格）" onClick={() => onAsk(EXPLAIN_SELECTION, { submit: true, context: readerContext(selection.text) })}><Sparkles size={14} />解释这段<kbd>空格</kbd></button><button onClick={() => highlight('yellow').catch(e => notify(e.message))}><MessageSquarePlus size={14} />写旁注</button><span className="tool-divider" />{['yellow', 'green', 'blue', 'pink'].map(c => <button key={c} className={`color-dot ${c}`} aria-label={`${{ yellow: '黄色', green: '绿色', blue: '蓝色', pink: '粉色' }[c]}高亮`} onClick={() => highlight(c).catch(e => notify(e.message))} />)}<button aria-label="取消选区" onClick={() => { setSelection(null); window.getSelection()?.removeAllRanges(); }}><X size={14} /></button></div>}
    <div className="pdf-status"><span><Highlighter size={12} />选中原文 → 写旁注 · 空格问 AI</span><span>{indexed < Math.min(doc.numPages, 800) ? `解析全文 ${indexed}/${doc.numPages}` : doc.numPages > 800 ? '已索引前 800 页' : '全文已就绪'} · PDF 第 {page} 页</span></div>
  </div>;
}
