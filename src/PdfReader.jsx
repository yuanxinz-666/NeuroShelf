...
export default function PdfReader({ paper, update, onContext, onAsk, onImport, notify, jump, mutateHighlight, onAnnotation, activeAnnotation, focus = false, toggleFocus, onNewSelection, dismissPopup, onSave, onHistory, onHelp }) {
  ...
  const continuousRef = useRef(null), currentPage = useRef(page);
  ...
  function focusSearch() {
    ...
    const input = document.querySelector('.pdf-search input');
    input?.focus();
    if (document.activeElement === input) { input.select(); pendingFind.current = false; }
  }
  useLayoutEffect(focusSearch, [chrome, focus]);
  ...
  useEffect(() => {
    if (focus || !doc || !canvasRef.current || !layerRef.current) return;
  ...
  useEffect(() => {
    if (focus) {
      setSelection(null);
      const info = continuousRef.current?.getPage(currentPage.current);
      setRendering(!info?.ready || info.width !== Math.min(width, 1200) * zoom);
    }
  }, [focus, width, zoom]);
  function pageInfo(number = page) {
    return focus ? continuousRef.current?.getPage(number) : { element: pageRef.current, layer: layerRef.current, canvas: canvasRef.current, text, ready: !rendering };
  }
  function continuousPage(number, info, keepSelection = false) {
    if (currentPage.current !== number) {
      currentPage.current = number; setPage(number); if (!keepSelection) { setSelection(null); window.getSelection()?.removeAllRanges(); }
      updateRef.current(paper.id, { page: number }).catch(() => {});
    }
    if (info?.text) texts.current[number] = info.text;
    setText(info?.text || texts.current[number] || ''); setRendering(!info?.ready && !info?.error);
  }
  function continuousReady(number, info) {
    if (!focus) return;
    if (info?.text) texts.current[number] = info.text;
    if (number === currentPage.current) { setText(info?.text || texts.current[number] || ''); setRendering(!info?.ready && !info?.error); }
  }
  function readerContext(excerpt = selection?.text || '', number = selection?.page || page) {
    return { page: number, totalPages: doc?.numPages || 0, pageText: pageInfo(number)?.text || texts.current[number] || '', selection: excerpt, source: 'pdf', anchor: selection?.anchor,
      getRelated: query => relatedPages(texts.current, query, number),
      getImage: () => {
        const info = pageInfo(number), original = info?.canvas;
        if (!original?.width || !info.ready) throw new Error(t("请等待当前页显示完成。"));
  ...
  useEffect(() => {
    const explainOnSpace = event => {
      ...
      if (!selected || !pageInfo(selected.page || page)?.ready) return;
      event.preventDefault();
      if (event.repeat) return;
      setSelection(selected);
      onAsk(t(EXPLAIN_SELECTION), { submit: true, context: { ...readerContext(selected.text, selected.page || page), anchor: selected.anchor } });
    };
  ...
  useEffect(() => {
    ...
    if (focus) { if (highlight?.rects?.[0]) continuousRef.current?.go(page, Math.max(0, highlight.rects[0].y - .05)); return; }
  ...
  useEffect(() => {
    const command = event => {
      ...
      if (event.detail === 'find') { pendingFind.current = true; wakeChrome(); focusSearch(); }
    };
    window.addEventListener('neuroshelf:pdf-command',command); return () => window.removeEventListener('neuroshelf:pdf-command',command);
  }, [doc,page,focus]);
  function go(number) {
    if (!doc) return;
    const n = Math.max(1, Math.min(doc.numPages, Math.trunc(Number(number) || 1)));
    setPageInput(String(n));
    if (focus) { setError(''); setSelection(null); window.getSelection()?.removeAllRanges(); continuousRef.current?.go(n); return; }
  ...
  function readSelection() {
    ...
    const selectedElement = selected.anchorNode?.nodeType === 1 ? selected.anchorNode : selected.anchorNode?.parentElement;
    const number = focus ? Number(selectedElement?.closest('[data-pdf-page]')?.dataset.pdfPage) : page;
    const info = pageInfo(number);
    if (!info?.ready || !info.layer?.contains(selected.anchorNode) || !info.layer?.contains(selected.focusNode)) return null;
    const range = selected.getRangeAt(0), bounds = info.element.getBoundingClientRect();
    ...
    return { page: number, text: selected.toString().trim().slice(0, 20000), rects, anchor: { x: screen.left + screen.width / 2, top: screen.top, bottom: screen.bottom } };
  }
  function captureSelection(event, number = page) {
    const selected = readSelection();
    if (selected && focus) continuousPage(selected.page, pageInfo(selected.page), true);
    setSelection(selected); if (selected) onNewSelection?.();
    const info = pageInfo(number);
    if (selected || !info?.ready || !Number.isFinite(event.clientX)) return;
    ...
    const hit = (paper.highlights || []).find(h => h.source === 'pdf' && h.page === number && (!h.pdfHash || h.pdfHash === paper.pdf.hash) && h.rects?.some(r => x >= r.x && x <= r.x + r.w && y >= r.y && y <= r.y + r.h));
    if (hit) {
      if (focus) { const r = hit.rects[0]; continuousPage(number, info); setSelection({page:number,text:hit.text,rects:hit.rects,highlightId:hit.id,color:hit.color,anchor:{x:bounds.left+(r.x+r.w/2)*bounds.width,top:bounds.top+r.y*bounds.height,bottom:bounds.top+(r.y+r.h)*bounds.height}}); }
      onAnnotation(hit, true);
    }
  }
  async function highlight(color, comment = false) {
    if (!selection || addingHighlight.current || !pageInfo(selection.page || page)?.ready) return;
    addingHighlight.current = true;
    const highlight = selection.highlightId ? paper.highlights.find(h => h.id === selection.highlightId) : { id: crypto.randomUUID(), source: 'pdf', pdfHash: paper.pdf.hash, page: selection.page || page, text: selection.text, rects: selection.rects, color, comment: '' };
    try {
  ...
  return <div className="pdf-reader" onMouseMove={e => { if (focus && e.clientY < 68) wakeChrome(); }}>
    ...
    {focus && focusHint && <div className="focus-hint">{t("滚轮连续阅读整篇 PDF · 右侧边缘打开 AI · 底部翻页 · F11 / Esc 退出")}</div>}
    <div className={`pdf-toolbar ${chrome ? "awake" : ""}`} onMouseEnter={wakeChrome}>
      <div className="toolbar-group"><button className="icon-button" aria-label={t("上一页")} disabled={page <= 1} onClick={() => go(page - 1)}><ChevronLeft size={17} /></button>
        <input aria-label={t("PDF 页码")} className="page-input" value={pageInput} onChange={e => setPageInput(e.target.value)} onBlur={() => { if (pageInput !== String(page)) go(pageInput); }} onKeyDown={e => { if (e.key === 'Enter') go(pageInput); }} />
    ...
    <div className="pdf-scroll" ref={scrollRef} tabIndex={0} onMouseDown={e => { if (focus && e.target === e.currentTarget) { setSelection(null); dismissPopup?.(); } }} onScroll={() => { if (focus && selection) { const bounds=pageInfo(selection.page || page)?.element?.getBoundingClientRect(), r=selection.rects.at(-1); if (bounds && r) setSelection(old => ({...old,anchor:{x:bounds.left+(r.x+r.w/2)*bounds.width,top:bounds.top+r.y*bounds.height,bottom:bounds.top+(r.y+r.h)*bounds.height}})); } }}>
      {focus ? <ContinuousPdf ref={continuousRef} doc={doc} width={Math.min(width, 1200) * zoom} initialPage={page} page={page} paper={paper} scrollRef={scrollRef} onPage={continuousPage} onReady={continuousReady} capture={captureSelection} annotate={onAnnotation} activeAnnotation={activeAnnotation} /> : <><div className="pdf-page" ref={pageRef} onMouseUp={captureSelection} onTouchEnd={captureSelection}>
      ...
      {!focus && !rendering && !text.trim() && <div className="scan-notice"><ImageIcon size={16} />{t("此页没有可选文字。可在右侧勾选「附上当前页图片」，让 AI 解释扫描页或图表。")}</div>}
    </div>
    {focus && <nav className="focus-pagination" aria-label={t('专注阅读翻页')}>
      <button aria-label={t('专注模式上一页')} title="← / Page Up" disabled={page <= 1} onClick={() => { go(page - 1); scrollRef.current?.focus({preventScroll:true}); }}><ChevronLeft size={18} />{t('上一页')}</button>
      <label><input aria-label={t('专注模式页码')} value={pageInput} onChange={e => setPageInput(e.target.value)} onBlur={() => { if (pageInput !== String(page)) go(pageInput); }} onKeyDown={e => { if (e.key === 'Enter') { go(pageInput); scrollRef.current?.focus({preventScroll:true}); } }} /><span>/ {doc.numPages}</span></label>
      <button aria-label={t('专注模式下一页')} title="→ / Page Down" disabled={page >= doc.numPages} onClick={() => { go(page + 1); scrollRef.current?.focus({preventScroll:true}); }}>{t('下一页')}<ChevronRight size={18} /></button><span className="continuous-reading-label">{t('滚轮连续阅读')}</span>
    </nav>}
