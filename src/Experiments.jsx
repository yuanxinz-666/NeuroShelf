...
import { BookOpen, FlaskConical, Plus, GitBranch, ImagePlus, Image as ImageIcon, Check, Circle, CircleDot, CircleAlert, Archive, RotateCcw, X, Minus, Maximize, Download, List, LoaderCircle, ChevronDown, ChevronRight, Save } from 'lucide-react';
...
import { ExperimentReferences } from './ExperimentReferences';
...
function ExperimentEditor({ node, nodes, project, papers, open, change, importImage, action, busy, add, confirm, preview, notify }) {
  ...
  return <aside className="exp-editor" aria-label={t("实验步骤详情")} data-node-id={node.id} onPaste={e => { if (e.clipboardData.files.length) { e.preventDefault(); upload(e.clipboardData.files); } }}>
    ...
    <div className="exp-editor-body">
      ...
      <ExperimentReferences node={node} papers={papers} projectId={project.id} change={change} action={action} busy={busy} open={open} />
...
function ProgressSummary({ projectId, node, change }) {
  ...
  return <div className="exp-node-progress">
    ...
    <div className="exp-node-footer"><span className="exp-node-meta">{node.date || t('未设日期')}{node.references?.length > 0 && <span title={t('实验参考文献')}><BookOpen size={12} />{node.references.length}</span>}{node.evidence.length > 0 && <span><ImageIcon size={12} />{node.evidence.length}</span>}</span><SaveState draft={draft} /></div>
...
function exportRecords(project, nodes, papers) {
  ...
  const visit = (parent, depth) => nodes.filter(n => n.parentId === parent).forEach(n => {
    ...
    if (n.references?.length) {
      lines.push(t('实验参考文献'), '');
      n.references.forEach(ref => { const paper = papers.find(p => p.id === ref.paperId); lines.push('- ' + (paper?.title || ref.paperId), '  ' + ref.roles.map(role => t(roles[role])).join(' · '), ...(paper?.url ? ['  ' + paper.url] : []), ...(ref.note ? ['  ' + ref.note.replace(/\n/g, '\n  ')] : []), ''); });
    }
...
export default function Experiments({ project, data, papers, open, initialSelectedId = null, onSelect, change, importImage, query = '', notify }) {
  ...
  const [selectedId, setSelected] = useState(initialSelectedId), [mode, setMode] = useState('map'), [collapsed, setCollapsed] = useState(new Set()), [busy, setBusy] = useState(false), [confirmation, setConfirmation] = useState(null), [preview, setPreview] = useState(null), [archives, setArchives] = useState(false);
  ...
  return <section className="experiments-page">
    <header className="exp-heading"><div><span className="page-eyebrow">EXPERIMENT JOURNEY</span><h1>{t("实验进程")}</h1><p>{t("把问题、实验与证据，连成自己的研究路线。")}</p></div><div className="exp-heading-actions"><button className="icon-button" title={t("导出实验记录 Markdown")} aria-label={t("导出实验记录")} disabled={!nodes.length || busy} onClick={() => action(async () => { const snapshot = await api.load(); exportRecords(project, snapshot.experiments?.nodes.filter(n => !n.archived) || [], snapshot.papers); })}><Download size={18} /></button><button className="button" disabled={busy} onClick={() => action(() => setArchives(true))}><Archive size={15} />{t("归档")}{archiveRoots.length > 0 && ` (${archiveRoots.length})`}</button><button className="button primary" disabled={busy} onClick={() => add(null)}><Plus size={16} />{t("新建实验")}</button></div></header>
