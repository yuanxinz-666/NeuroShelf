...
function AnnotationCard({ paperId, highlight, active, mutate, onLocate, notify }) {
  ...
  const save = () => persist().catch(error => notify(t("旁注保存失败：") + error.message));
  return <article ref={card} className={`annotation-card ${highlight.color || 'yellow'} ${active?.id === highlight.id ? 'active' : ''}`} data-highlight-id={highlight.id}>
    <header><button className="annotation-location" onClick={() => onLocate(highlight)}><Highlighter size={13} />{highlight.source === 'pdf' ? tx`PDF 第 ${highlight.page} 页` : t("原网页摘录")}<ArrowUpRight size={13} /></button><button className="icon-button" aria-label={t("删除这条旁注和高亮")} title={t("删除这条旁注和高亮")} onClick={() => setConfirmDelete(true)}><Trash2 size={14} /></button></header>
    <button className="annotation-quote" title={t("定位这段原文")} onClick={() => onLocate(highlight)}>{highlight.text}</button>
    <textarea ref={input} aria-label={tx`第 ${highlight.page || 0} 页旁注`} placeholder={t("写下你的理解、疑问或修改意见…")} value={comment} maxLength={100000} onChange={event => {
    ...
    <footer><span className={status === '保存失败' ? 'failed' : ''} role="status"><Check size={12} />{te(status)}</span>{status === '保存失败' && <button onClick={save}>{t("重试保存")}</button>}</footer>
    {confirmDelete && <div className="annotation-delete"><span>{t("删除这条旁注和高亮？")}</span><button onClick={() => setConfirmDelete(false)}>{t("取消")}</button><button onClick={async () => {
...
export default function AnnotationPanel({ paper, active, mutate, onLocate, notify, visible }) {
  ...
  return <section id="annotation-panel" className="annotation-panel" role="tabpanel" aria-labelledby="annotations-tab" hidden={!visible}>
    <div className="annotation-heading"><h2>{t("把想法留在原文旁")}</h2><p>{t("每条旁注对应一段高亮，自动保存在本机。")}</p></div>
    <div className="annotation-list">
      {!highlights.length && <div className="annotation-empty"><MessageSquareText size={30} /><h3>{t("从一段原文开始")}</h3><p>{t("用鼠标选中 PDF 文字，点击「写旁注」，就能在这里记录想法。")}</p></div>}
